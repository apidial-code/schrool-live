 ...
 */
export async function createMagicLinkToken(userId, expiryMinutes = 60) {
...
export async function verifyMagicLinkToken(token) {
    ...
    if (token.includes('.')) {
        try {
            ...
            const decoded = jwt.verify(token, ENV.jwtSecret || 'demo-secret');
    ...
    const [tokenRecord] = await db
        ...
        .where(and(eq(magicLinkTokens.token, token), eq(magicLinkTokens.used, 0), gt(magicLinkTokens.expiresAt, now)))
        .limit(1);
    if (!tokenRecord) {
        return null;
    }
 ...
 */
export async function generateMagicLink(userId, baseUrl) {

...
router.get("/", async (req, res) => {
    try {
        const [rows] = await db().query(
            `SELECT id, title, description, videoId, thumbnailUrl, duration, orderIndex, category FROM lessons ORDER BY orderIndex ASC`
        );
        res.json(rows);
    } catch (err) {
...
router.get("/:id", async (req, res) => {
    try {
        ...
        const [rows] = await db().query(
            `SELECT id, title, description, videoId, thumbnailUrl, duration, orderIndex, category FROM lessons WHERE id = ? LIMIT 1`,
            [id]
        );
...
router.get("/:id/exercises", async (req, res) => {
    try {
        ...
        let query = `SELECT id, lessonId, question, correctAnswer, solutionVideoId, orderIndex, difficulty, options, imageUrl FROM exercises WHERE lessonId = ?`;

...
import { verifyMagicLinkToken, createMagicLinkToken } from "../_core/magicLink";
...
router.get("/magic", async (req, res) => {
  ...
  try {
    ...
    if (!user && process.env.MOCK_DB_ENABLED === 'true') {
      ...
      if (!(global as any).mockData.users) (global as any).mockData.users = [];
      (global as any).mockData.users.push(user);
    }

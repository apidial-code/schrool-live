...
export function TeacherTraining() {
    ...
    const { data: codeValidation, isLoading: isValidating } = trpc.teacherOnboarding.validateApprovalCode.useQuery({ approvalCode: code || "" }, { enabled: !!code });
    // Get exam attempts
    const { data: attempts } = trpc.teacherOnboarding.getExamAttempts.useQuery({ email: codeValidation?.email || "" }, { enabled: !!codeValidation?.email });
    useEffect(() => {
        if (attempts) {
    ...
    if (isValidating) {
        return (_jsx("div", { className: "min-h-screen flex items-center justify-center bg-background", children: _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4" }), _jsx("p", { className: "text-muted-foreground", children: "Validating your approval code..." })] }) }));
    }
    if (!codeValidation?.success) {
        return (_jsx("div", { className: "min-h-screen flex items-center justify-center bg-background p-4", children: _jsxs(Card, { className: "w-full max-w-md", children: [_jsx(CardHeader, { children: _jsx(CardTitle, { className: "text-destructive", children: "Invalid Approval Code" }) }), _jsxs(CardContent, { children: [_jsx("p", { className: "text-sm text-muted-foreground mb-4", children: "The approval code you provided is invalid or has expired. Please check your email for the correct code." }), _jsx(Button, { onClick: () => setLocation("/"), className: "w-full", children: "Return to Home" })] })] }) }));
    }
    // Stage 1: Critical Warning
    if (stage === "warning") {
        return (_jsx("div", { className: "min-h-screen flex items-center justify-center bg-background p-4", children: _jsxs(Card, { className: "w-full max-w-2xl", children: [_jsx(CardHeader, { children: _jsxs(CardTitle, { className: "flex items-center gap-2 text-destructive", children: [_jsx(AlertCircle, { className: "h-6 w-6" }), "IMPORTANT: Certification Requirements"] }) }), _jsxs(CardContent, { className: "space-y-4", children: [_jsxs(Alert, { variant: "destructive", children: [_jsx(AlertCircle, { className: "h-4 w-4" }), _jsx(AlertTitle, { children: "Critical Information" }), _jsx(AlertDescription, { children: _jsxs("ul", { className: "list-disc list-inside space-y-2 mt-2", children: [_jsx("li", { children: "You have UP TO 5 ATTEMPTS to pass the certification exam" }), _jsx("li", { children: "You must achieve 80% or higher to pass (36 out of 45 questions)" }), _jsx("li", { children: "After 5 failed attempts, your application will be PERMANENTLY REJECTED" }), _jsx("li", { children: "Please study the course materials carefully before attempting the exam" })] }) })] }), _jsxs("div", { className: "bg-muted p-4 rounded-lg", children: [_jsx("h3", { className: "font-semibold mb-3", children: "Course Overview:" }), _jsx("div", { className: "space-y-2", children: COURSE_MODULES.map((module) => (_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(BookOpen, { className: "h-4 w-4 text-primary" }), _jsxs("div", { className: "flex-1", children: [_jsx("p", { className: "text-sm font-medium", children: module.title }), _jsxs("p", { className: "text-xs text-muted-foreground", children: [module.questions, " exam questions"] })] })] }, module.id))) })] }), _jsxs("div", { className: "bg-blue-50 border border-blue-200 rounded-lg p-3", children: [_jsxs("p", { className: "text-sm text-blue-900", children: [_jsx("strong", { children: "Total Exam:" }), " 45 questions covering all three modules"] }), _jsxs("p", { className: "text-sm text-blue-900", children: [_jsx("strong", { children: "Passing Score:" }), " 80% (36 correct answers)"] })] }), _jsx("div", { className: "space-y-2", children: _jsxs("label", { className: "flex items-center gap-2", children: [_jsx("input", { type: "checkbox", checked: warningAcknowledged, onChange: (e) => setWarningAcknowledged(e.target.checked), className: "w-4 h-4" }), _jsx("span", { className: "text-sm", children: "I understand the requirements and am ready to proceed" })] }) }), _jsx(Button, { onClick: () => setStage("training"), disabled: !warningAcknowledged, className: "w-full", size: "lg", children: "Start Training Course" })] })] }) }));
    }
    // Stage 2: Training Modules
    if (stage === "training") {
        const allCompleted = completedModules.length === COURSE_MODULES.length;
        return (_jsx("div", { className: "min-h-screen bg-background p-4", children: _jsxs("div", { className: "max-w-4xl mx-auto", children: [_jsxs("div", { className: "mb-6", children: [_jsx("h1", { className: "text-3xl font-bold mb-2", children: "Teacher Certification Training" }), _jsx("p", { className: "text-muted-foreground", children: "Complete all course modules before taking the certification exam" })] }), _jsxs("div", { className: "mb-6", children: [_jsxs("div", { className: "flex items-center justify-between mb-2", children: [_jsx("span", { className: "text-sm font-medium", children: "Progress" }), _jsxs("span", { className: "text-sm text-muted-foreground", children: [completedModules.length, " of ", COURSE_MODULES.length, " completed"] })] }), _jsx(Progress, { value: (completedModules.length / COURSE_MODULES.length) * 100 })] }), _jsx("div", { className: "space-y-4", children: COURSE_MODULES.map((module) => {
                            const isCompleted = completedModules.includes(module.id);
                            return (_jsxs(Card, { className: isCompleted ? "border-green-500 bg-green-50" : "", children: [_jsx(CardHeader, { children: _jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { className: "flex-1", children: [_jsxs(CardTitle, { className: "flex items-center gap-2", children: [isCompleted && _jsx(CheckCircle2, { className: "h-5 w-5 text-green-600" }), module.title] }), _jsx(CardDescription, { children: module.description })] }), _jsxs("div", { className: "flex flex-col items-end gap-2 text-sm text-muted-foreground", children: [_jsxs("div", { className: "flex items-center gap-1", children: [_jsx(Clock, { className: "h-4 w-4" }), module.duration] }), _jsxs("div", { className: "text-xs bg-muted px-2 py-1 rounded", children: [module.questions, " questions"] })] })] }) }), _jsx(CardContent, { children: isCompleted ? (_jsxs("div", { className: "flex items-center gap-2 text-green-600 text-sm font-medium", children: [_jsx(CheckCircle2, { className: "h-4 w-4" }), "Module Completed"] })) : (_jsx(Button, { onClick: () => setCompletedModules([...completedModules, module.id]), variant: "outline", children: "Mark as Completed" })) })] }, module.id));
                        }) }), _jsxs(Button, { onClick: () => {
                            setCurrentQuestionIndex(0);
                            setStage("exam");
                        }, disabled: !allCompleted, className: "w-full mt-6", size: "lg", children: ["Proceed to Certification Exam ", _jsx(ChevronRight, { className: "h-4 w-4 ml-2" })] })] }) }));
    }
    ...
    if (stage === "exam") {
        ...
        return (_jsx("div", { className: "min-h-screen bg-background p-4", children: _jsxs("div", { className: "max-w-3xl mx-auto", children: [_jsxs("div", { className: "mb-6", children: [_jsxs("div", { className: "flex items-center justify-between mb-4", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-2xl font-bold", children: "Certification Exam" }), _jsxs("p", { className: "text-sm text-muted-foreground", children: ["Question ", currentQuestionIndex + 1, " of ", EXAM_QUESTIONS.length] })] }), _jsxs("div", { className: "text-right", children: [_jsxs("p", { className: "text-sm text-muted-foreground", children: ["Attempt ", currentAttempt, " of 5"] }), _jsxs("p", { className: "text-sm font-medium text-destructive", children: [attemptsRemaining, " attempts remaining"] })] })] }), _jsx(Progress, { value: ((currentQuestionIndex + 1) / EXAM_QUESTIONS.length) * 100 })] }), _jsxs(Card, { className: "mb-6", children: [_jsxs(CardHeader, { children: [_jsx(CardTitle, { className: "text-lg", children: currentQuestion.question }), _jsx(CardDescription, { children: "Select the correct answer from the options below" })] }), _jsx(CardContent, { children: _jsx("div", { className: "space-y-3", children: currentQuestion.options.map((option) => (_jsxs("label", { className: "flex items-center gap-3 p-3 rounded-lg border cursor-pointer hover:bg-muted transition-colors", children: [_jsx("input", { type: "radio", name: `q${currentQuestion.id}`, value: option, checked: examAnswers[currentQuestion.id] === option, onChange: (e) => setExamAnswers({ ...examAnswers, [currentQuestion.id]: e.target.value }), className: "w-4 h-4" }), _jsx("span", { className: "text-sm", children: option })] }, option))) }) })] }), _jsxs("div", { className: "flex gap-3", children: [_jsx(Button, { onClick: () => setCurrentQuestionIndex(Math.max(0, currentQuestionIndex - 1)), disabled: currentQuestionIndex === 0, variant: "outline", className: "flex-1", children: "Previous" }), isLastQuestion ? (_jsx(Button, { onClick: () => {
                                    // Calculate score
                                    let correct = 0;
                                    ...
                                    setExamScore(score);
                                    setStage("results");
                                }, disabled: !allAnswered, className: "flex-1", children: "Submit Exam" })) : (_jsx(Button, { onClick: () => setCurrentQuestionIndex(currentQuestionIndex + 1), disabled: !examAnswers[currentQuestion.id], className: "flex-1", children: "Next" }))] }), _jsx("div", { className: "mt-6 p-4 bg-muted rounded-lg", children: _jsxs("p", { className: "text-xs text-muted-foreground text-center", children: ["Progress: ", Object.keys(examAnswers).length, " of ", EXAM_QUESTIONS.length, " questions answered"] }) })] }) }));
    }
    // Stage 4: Results
    if (stage === "results" && examScore !== null) {
        ...
        return (_jsx("div", { className: "min-h-screen flex items-center justify-center bg-background p-4", children: _jsxs(Card, { className: "w-full max-w-md", children: [_jsx(CardHeader, { children: _jsxs(CardTitle, { className: `flex items-center gap-2 ${passed ? "text-green-600" : "text-destructive"}`, children: [passed ? _jsx(CheckCircle2, { className: "h-6 w-6" }) : _jsx(AlertCircle, { className: "h-6 w-6" }), passed ? "Exam Passed!" : "Exam Failed"] }) }), _jsxs(CardContent, { className: "space-y-4", children: [_jsxs("div", { className: "text-center", children: [_jsxs("p", { className: "text-4xl font-bold mb-2", children: [Math.round(examScore), "%"] }), _jsxs("p", { className: "text-muted-foreground text-sm", children: [correct, " out of ", EXAM_QUESTIONS.length, " questions correct"] })] }), _jsxs(Alert, { className: passed ? "border-green-500 bg-green-50" : "border-destructive bg-destructive/10", children: [_jsx(AlertCircle, { className: `h-4 w-4 ${passed ? "text-green-600" : "text-destructive"}` }), _jsx(AlertTitle, { children: passed ? "Congratulations!" : "Next Steps" }), _jsx(AlertDescription, { children: passed
                                            ? "You have passed the certification exam. You will receive the teacher agreement form via email shortly."
                                            : `You scored ${Math.round(examScore)}%. You need 80% to pass. You have ${attemptsRemaining - 1} attempts remaining.` })] }), passed ? (_jsx(Button, { onClick: () => setLocation("/"), className: "w-full", children: "Return to Home" })) : attemptsRemaining > 1 ? (_jsxs("div", { className: "space-y-2", children: [_jsx(Button, { onClick: () => {
                                            setExamAnswers({});
                                            setExamScore(null);
                                            setCurrentQuestionIndex(0);
                                            setStage("exam");
                                        ...
                                        }, className: "w-full", children: "Retake Exam" }), _jsx(Button, { onClick: () => setLocation("/"), variant: "outline", className: "w-full", children: "Return to Home" })] })) : (_jsxs("div", { className: "space-y-2", children: [_jsxs(Alert, { variant: "destructive", children: [_jsx(AlertCircle, { className: "h-4 w-4" }), _jsx(AlertTitle, { children: "Application Rejected" }), _jsx(AlertDescription, { children: "You have exceeded the maximum number of attempts. Your application has been permanently rejected." })] }), _jsx(Button, { onClick: () => setLocation("/"), className: "w-full", children: "Return to Home" })] }))] })] }) }));
    }
    return null;
}
